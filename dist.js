...
const opts = {
  ...
  config: {
    ...
    productName: 'Translatium Legacy',
    files: [
      '!docs/**/*',
      '!popclip/**/*',
      '!test/**/*',
    ...
    afterAllArtifactBuild: () => {
      ...
      const appxBundlePath = path.join('dist', `Translatium Legacy ${appVersion}.appxbundle`);
      const appxPath = path.join(__dirname, 'dist', `Translatium Legacy ${appVersion}.appx`);
      const bundleDirPath = path.join(__dirname, 'dist', 'appx_bundle');
      const appxDestPath = path.join(bundleDirPath, 'Translatium Legacy.appx');
